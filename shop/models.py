...
class Order(models.Model):
    ...
    status = models.CharField(max_length=50, default='pending')  # Order status can be 'pending', 'shipped', etc.
    ...
    def update_status(self, new_status):
        """
        Update the order status and perform actions like stock update.
        """
        # Ensure product stock is updated when an order status is set to 'shipped' or 'delivered'
        if new_status in ['shipped', 'delivered']:
            # Ensure there is enough stock
            for order_item in self.order_items.all():  # Assuming Order has a relation to products/items
                product = order_item.product
                product.update_stock(order_item.quantity)  # Update the stock of the product

        self.status = new_status
        self.save()
        self.notify_seller(new_status)

    def notify_seller(self, status):
        """
        Notify the seller when the order status changes.
        Sends email to the seller for significant status updates.
        """
        seller_email = self.order_items.first().product.seller.email  # Assuming there's a relation from OrderItem to Product
        send_mail(
            'Order Status Update',
            f'The status of your order #{self.id} has changed to: {status}',
            'from@example.com',  # Replace with your system email
            [seller_email],
            fail_silently=False,
        )
...
class OrderItem(models.Model):
    STATUS_CHOICES = [
        ...
        ('Shipped', 'Shipped'),
        ('Delivered', 'Delivered'),
        ('Cancelled', 'Cancelled'),  # Optionally add Cancelled status
        ('Returned', 'Returned'),  # Optionally add Returned status
    ]
    ...
    def update_stock_on_status_change(self):
        """
        Automatically update product stock based on the order item status.
        For example, reduce stock when shipped or delivered.
        """
        if self.status == 'Shipped' or self.status == 'Delivered':
            # Decrease the product's stock when the order item is shipped or delivered
            self.product.update_stock(self.quantity)
        elif self.status == 'Cancelled' or self.status == 'Returned':
            # Optionally, restock if the order is cancelled or returned
            self.product.restock(self.quantity)

    def __str__(self):
        return f"{self.quantity} x {self.product.name} (Order {self.order.id})"

    # Optionally use Django signals to update stock when status changes
...
class Notification(models.Model):
    ...
    def __str__(self):
        return f"Notification for {self.seller.username} regarding {self.product.name}"

    # Optional: Method to mark as read
    def mark_as_read(self):
        self.is_read = True
        self.save()

    # Optional: Class method to create a new notification
    @classmethod
    def create_notification(cls, seller, product, message):
